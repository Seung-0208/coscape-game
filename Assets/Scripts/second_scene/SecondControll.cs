...
public class SecondControll : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        Cam1 = GameObject.Find("Cam1").GetComponent<Animator>(); //애니메이션
        //카메라 위치 설정
    ...
    void MoveObs()
    {
        ...
        if (curpos==cam2pos || curpos == cam4pos)
        {
            ...
            if (Cam1.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1.0f&&curpos==cam2pos) {
                ...
                for (int i=0;i<obstacle.Length;i++) {
                    ...
                    if (i==obstacle.Length-3) newPosition.z = playItem[0].position.z - 0.5f;
                    else if (i==obstacle.Length-2) newPosition.z = playItem[0].position.z - 0.51f;
        ...
        else if (curpos == cam1pos || curpos == cam3pos)
        {
            ...
            if (Cam1.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1.0f&&curpos==cam3pos) {
                ...
                for (int i=0;i<obstacle.Length;i++) {
                    newPosition = obstacle[i].position;
                    if (i==obstacle.Length-2||i==obstacle.Length-3) newPosition.x=playItem[0].position.x - 0.5f;
                    else newPosition.x = playItem[0].position.x;
                    obstacle[i].position = newPosition;
                }
                if (x_cur + 0.1f <= gameObject.transform.position.x|| x_cur-0.2f >= gameObject.transform.position.x || isInsideModel) { //**튕겨나가기
                    warning.SetActive(true);
                    comeback();
                    outPosition.x = 8f;
                    gameObject.transform.position = outPosition;
                ...
                if (x_cur + 0.1f <= gameObject.transform.position.x|| x_cur-0.2f >= gameObject.transform.position.x || isInsideModel) {
                    warning.SetActive(true);
                    comeback();
                    outPosition.x = -1f;
                    gameObject.transform.position = outPosition;
