...
public class Ice2Controll : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Cam1.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1.0f && Input.GetKeyDown(KeyCode.W)) {
            ...
            //     isInsideModel = IsObjectInsideModel();
            //     if (isInsideModel) LoadScene();
            // }
        }
        ...
        if(Cam1.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1.0f && Input.GetKeyDown(KeyCode.Q)) {
            ...
            //isInsideModel = IsObjectInsideModel();
            // Invoke("IsObjectInsideModel", 0.5f);
            // if (isInsideModel) LoadScene();
        } 
    ...
    void SetCamera()
    {
        if (curpos==cam2pos || curpos == cam4pos)
        {
            ...
            if (Cam1.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1.0f&&curpos==cam2pos) {
                ...
                for (int i=0;i<obstacle.Length;i++) { //기둥들 위치 변경
                    ...
                    if (i==obstacle.Length-2 || i==obstacle.Length-3) newPosition.x = playItem[0].position.x - 0.72f;
                    else newPosition.x = playItem[0].position.x; //캐릭터 위치의 z값으로 위치를 맞춤
                    obstacle[i].position = newPosition;
                    //if (coll) Debug.Log("collision!");   
                }
                //IsObjectInsideModel();
        ...
        else if (curpos == cam1pos || curpos == cam3pos)
        {
            ...
            if (Cam1.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1.0f&&curpos==cam3pos) {
                ...
                for (int i=0;i<obstacle.Length;i++) {
                    newPosition = obstacle[i].position;
                    if (i==obstacle.Length-3) newPosition.z = playItem[0].position.z + 5.07f;
                    else if (i==obstacle.Length-2) newPosition.z = playItem[0].position.z + 4.05f;
                ...
                for (int i=0;i<obstacle.Length;i++) {
                    newPosition = obstacle[i].position;
                    if (i==obstacle.Length-2) newPosition.z = playItem[0].position.z + 4.01f;
                    else if (i==obstacle.Length-3) newPosition.z = playItem[0].position.z + 4.5f;
                    else newPosition.z = playItem[0].position.z;
                    obstacle[i].position = newPosition;
                    //if (coll) Debug.Log("collision");
                }
    ...
    void OnCollisionEnter(Collision other) { //움직임 멈춤
