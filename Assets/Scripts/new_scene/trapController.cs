...
public class trapController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        //trap 경고창----------------------
        trap_warning = GameObject.Find("trap_warning");
        trap_warning.SetActive(false);
        //--------------------------------
    ...
    void SetCamera()
    {
        ...
        if (curpos==cam2pos || curpos == cam4pos)
        {
            ...
            if (Cam1.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1.0f&&curpos==cam2pos) {
                ...
                for (int i=0;i<obstacle.Length;i++) { //기둥들 위치 변경
                    ...
                    if (i==obstacle.Length-1) newPosition.x = playItem[0].position.x - 0.62f;
                    else if (i==obstacle.Length-5) newPosition.x = playItem[0].position.x - 3f;
                ...
                for (int i=0;i<obstacle.Length;i++) {
                    newPosition = obstacle[i].position;
                    if (i==obstacle.Length-1) newPosition.x = playItem[0].position.x - 0.85f;
        ...
        else if (curpos == cam1pos || curpos == cam3pos)
        {
            ...
            if (Cam1.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1.0f&&curpos==cam3pos) {
                ...
                for (int i=0;i<obstacle.Length;i++) {
                    newPosition = obstacle[i].position;
                    if (i==obstacle.Length-1) newPosition.z = playItem[0].position.z + 4.1f;
                ...
                for (int i=0;i<obstacle.Length;i++) {
                    newPosition = obstacle[i].position;
                    if (i==obstacle.Length-1) newPosition.z = playItem[0].position.z + 4.04f;
    ...
    void OnCollisionEnter(Collision other) {
        if (other.gameObject.CompareTag("trap")) {
            gameObject.GetComponent<Animator>().Play("Body animation",0,0f);
            trap_warning.SetActive(true);
            Invoke("LoadScene",2f);
        }            
    }
}
