...
public class Ice1Controll : MonoBehaviour
{
    ...
    void SetCamera()
    {
        ...
        if (curpos==cam2pos || curpos == cam4pos)
        {
            ...
            if (Cam1.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1.0f&&curpos==cam2pos) {
                ...
                for (int i=0;i<obstacle.Length;i++) { //기둥들 위치 변경
                    ...
                    if (i==obstacle.Length-1) newPosition.x = playItem[0].position.x - 0.75f;
                ...
                for (int i=0;i<obstacle.Length;i++) {
                    newPosition = obstacle[i].position;
                    if (i==obstacle.Length-1) newPosition.x = playItem[0].position.x-0.73f;
        ...
        else if (curpos == cam1pos || curpos == cam3pos)
        {
            ...
            if (Cam1.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1.0f&&curpos==cam3pos) {
                ...
                for (int i=0;i<obstacle.Length;i++) {
                    newPosition = obstacle[i].position;
                    if (i==obstacle.Length-1) newPosition.z = playItem[0].position.z + 3.86f;
                    else if (i==obstacle.Length-2) newPosition.z = playItem[0].position.z + 0.73f;
                ...
                for (int i=0;i<obstacle.Length;i++) {
                    newPosition = obstacle[i].position;
                    if (i==obstacle.Length-1) newPosition.z = playItem[0].position.z + 4f;
                    else newPosition.z = playItem[0].position.z;
                    obstacle[i].position = newPosition;
                }
                if (z_cur+0.1f <= gameObject.transform.position.z || z_cur-0.1f >= gameObject.transform.position.z || isInsideModel) { //**튕겨나가기
    ...
    void movingcheck() {
        RaycastHit hit;
        if (Physics.Raycast(transform.position, transform.right, out hit, 0.2f, collisionLayer)) {
